...
def sumForTimePeriod(indexXr):
    """
    calculates the sum for the period of interest defined in _indexDefitions for the indeces in _indexDefinitions
        ...
        if months[1]<12:
                answer.append(
                    xarray.concat(
                        ...
                        ).sum(dim='time') for year in yearRange], 
                        'year')
                    )
        # if the period goes over two years
        else:
